...
const summary = () => {
  ...
  const fetchAttendanceReport = async () => {
    try {
      const month = currentDate.month() + 1; // month() is zero-based
      const year = currentDate.year();
      const response = await axios.get(
        `http://192.168.8.150:8000/attendance-report-all-employees`,
        {
          params: {
            month: month,
            year: year,
    ...
    <ScrollView style={{ flex: 1, backgroundColor: "white" }}>
      ...
      <View style={{ marginHorizontal: 12 }}>
        {attendanceData.length > 0 ? (
          ...
          >
            No attendance data available for this month & year.
          </Text>
        )}
      </View>
    </ScrollView>
